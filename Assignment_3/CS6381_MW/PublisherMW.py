                    ...
                    if data:
                        ...
                        if self.current_broker_endpoint is None:
                            ...
                            try:
                                ...
                                self.logger.error("PublisherMW::watch_broker_primary - error disconnecting from old broker endpoint: " + str(ex))
                            self.pub.connect(new_broker_endpoint)
                            self.current_broker_endpoint = new_broker_endpoint
            else:
            ...
            if disc_resp.msg_type == discovery_pb2.TYPE_REGISTER:
                ...
                if self.current_broker_endpoint is None:
                    ...
                    try:
                        ...
                        self.logger.error("PublisherMW::handle_reply - error disconnecting from old broker: " + str(ex))
                    self.pub.connect(new_broker_endpoint)
                    self.current_broker_endpoint = new_broker_endpoint
                else:
