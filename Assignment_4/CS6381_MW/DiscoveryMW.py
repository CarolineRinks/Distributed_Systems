...
class DiscoveryMW:
    ...
    def update_state(self):
        """Write the current state dictionary into the state znode."""
        try:
    ...
    def handle_publisher_change(self, current_children):
        ...
        active_pub_ids = set(n.split("-",1)[0] for n in current_children)   # remove sequence from pub name
        self.logger.info(f"%%%%%%%%%%%%%% Handle publisher change. Active ids: {active_pub_ids}")
        self.logger.info(f"%%%%%%%%%%%%%% known_pub_ids - active_pub_ids: {known_pub_ids - active_pub_ids}")
        ...
        self.logger.info(f"DiscoveryMW::handle_spublisher_change - self.publishers:\n{self.publishers}")
        self.update_state()
    ...
    def handle_subscribers_change(self, current_children):
        ...
        active_sub_ids = set(n.split("-",1)[0] for n in current_children) 
        ...
        self.logger.info(f"DiscoveryMW::handle_subscribers_change - self.subscribers:\n{self.subscribers}")
        self.update_state()
        ...
        try:
            ...
            if role == discovery_pb2.ROLE_PUBLISHER:            
                self.publishers[entity_id] = {
                    ...
                    "history_size": list(register_req.info.history_size),
                    "ownership": register_req.info.ownership,
                    "topics": topics
                }
    ...
    def process_lookup_request(self, lookup_req):
        ...
        # Collect the publishers with matching topics
        matched = []
        for pub_id, pub_data in self.publishers.items():
            # check that ownership exists and is not None
            ownership = pub_data["ownership"]
            if ownership is None:
                self.logger.info(f"publisher {pub_id} has no ownership set, skipping")
                continue
            if len(lookup_req.topiclist) == 0 or any(topic in pub_data["topics"] for topic in lookup_req.topiclist):
                matched.append((pub_data["ownership"], pub_id, pub_data))
        # Sort the matching publishers by their ownership strength
        ...
        for ownership, pub_id, pub_data in matched:
            ...
            pub_info.history_size.extend(pub_data["history_size"])
            pub_info.ownership = pub_data["ownership"]
    ...
    def create_error_response(self, error_message):
        self.logger.error(f"1111111111111111111111111111111111 - DiscoveryMW::create_error_response - {error_message}")
        response = discovery_pb2.DiscoveryResp()
