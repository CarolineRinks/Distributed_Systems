...
class SubscriberAppln():
    ...
    def configure(self, args):
        try:
            ...
            # Get the history sizes requested by this subscriber
            history_list = args.history
            history_sizes = {}

            if len(history_list) != len(self.topiclist):
                for topic in self.topiclist:
                    history_sizes[topic] = int(history_list[0])      # use same history size for all topics if user didn't give multiple
            else:
                i = 0
                for topic in self.topiclist:
                    history_sizes[topic] = int(history_list[i])
                    i += 1

            # Configure a middleware object with the extracted info
            self.mw_obj = SubscriberMW(self.logger, self.topiclist, self.dissemination, self.group, history_sizes)
        ...
        try:
            ...
            if lookup_resp.status:
                ...
                if self.dissemination == "Direct":
                    ...
                    self.logger.info(f"SubscriberAppln::lookup_response :::::::::: {lookup_resp.publishers}")
...
def parseCmdLineArgs():
    ...
    parser.add_argument("-z", "--history", type=int, nargs="+", default=[10], help="Number(s) of past publications requested by subscriber")
